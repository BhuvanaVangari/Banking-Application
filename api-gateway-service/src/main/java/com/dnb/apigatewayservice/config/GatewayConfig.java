...
public class GatewayConfig {
	...
	public RouteLocator routes(RouteLocatorBuilder builder) {
		...
		return builder.routes()
				.route("AUTH-SERVICE", r -> r.path("/api/authenticate/**").filters(f -> f.filter(filter)).uri("lb://AUTH-SERVICE"))

				.route("ACCOUNT-SERVICE",
						r -> r.path("/api/account/**").filters(f -> f.filter(filter)).uri("lb://ACCOUNT-SERVICE"))

				.route("CREDIT-SERVICE",
						r -> r.path("/api/credit/**").filters(f -> f.filter(filter)).uri("lb://CREDIT-SERVICE"))
